...
contract Funds is Ownable {
  ...
  function getTimeRent(uint256[] memory _ids) external view returns (uint256[] memory) {
  ...
  function transferFunds(address _to, uint256 _amount) public onlyOwner {
    require(_amount <= address(this).balance, "There aren't enough funds to withdraw");
    (
      bool response, /*bytes memory data*/
